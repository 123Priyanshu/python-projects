import math
from tkinter import END, Button, Entry, Label
...
def button_equal():
    ...
    if math == "division":
        e.insert(0, f_num / int(second_number))
    if math == "square":
        e.insert(0, f_num * int(f_num))
    if math == "pi":
        e.insert(0, f_num * float(3.141592653589793238))
...
def button_pi():
    ...
    global math
    math = "pi"
    f_num = first_number(3.141592653589793238)
    e.delete(0, END)
    # Define Buttons
...
button_square = Button(root, text="x²", padx=38, pady=20,
                         command=button_square)
button_pi = Button(root,text="π", padx=38, pady=20,
                         command=button_square)
